...
let sendSimpleEmail = async (dataSend) => {
    ...
    if (dataSend.type === 'verifyEmail') {
        let info = await transporter.sendMail({
            from: '"Hieu1410 👻" <hieuoniichan@gmail.com>', // sender address
            to: dataSend.email, // list of receivers
            subject: "Xác thực email | MINHHIEUSHOP", // Subject line
    ...
    if (dataSend.type === 'forgotpassword') {
        let info = await transporter.sendMail({
            from: '"Hieu1410 👻" <hieuoniichan@gmail.com>', // sender address
            to: dataSend.email, // list of receivers
            subject: "Xác nhận quên mật khẩu | MINHHIEUSHOP", // Subject line
